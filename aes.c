...
void sub_bytes(uint32_t state[][CHANNELS]) {
    g256_newbasis_A2X(state[7], state[6], state[5], state[4],
                        state[3], state[2], state[1], state[0]);

    g256_inv(state[2], state[4], state[1], state[7],
                state[3], state[0], state[5], state[6]);
    
    g256_newbasis_X2S(state[3], state[0], state[5], state[6],
                        state[2], state[4], state[1], state[7]);
...
void shift_rows(uint32_t state[][CHANNELS], bool final) {
    ...
    for (int channel = 0; channel < CHANNELS; channel++) {
        for (int index = 0; index < REGISTER_NO; index++) {
            ...
            state[index][channel] &= 0xff00ffff;
            state[index][channel] |= (uint32_t)row << 16;

            // Row 2
            row = (state[index][channel] & 0x0000ff00) >> 8;
            row = (row << 4) | (row >> 4);

            state[index][channel] &= 0xffff00ff;
            state[index][channel] |= (uint32_t)row << 8;

            // Row 3
            row = state[index][channel] & 0x000000ff;
            row = (row << 6) | (row >> 2);

            state[index][channel] &= 0xffffff00;
            state[index][channel] |= (uint32_t)row;
        }
    }
...
void mix_columns(uint32_t state[][CHANNELS]) {
    uint32_t t[REGISTER_NO][CHANNELS], aux0[CHANNELS], aux1[CHANNELS];

    // t0
    ROTATE_LEFT_WORD(state[0], aux0, 8);
    XOR(state[0], aux0, t[0]);

    // t1
    ROTATE_LEFT_WORD(state[1], aux0, 8);
    ROTATE_LEFT_WORD(state[1], aux1, 16);
    XOR(aux0, aux1, t[1]);

    // t2
    ROTATE_LEFT_WORD(state[2], aux0, 8);
    XOR(state[2], aux0, t[2]);

    // t3
    ROTATE_LEFT_WORD(state[3], aux0, 8);
    ROTATE_LEFT_WORD(state[3], aux1, 16);
    XOR(aux0, aux1, t[3]);

    // t4
    ROTATE_LEFT_WORD(state[4], aux0, 8);
    XOR(state[4], aux0, t[4]);

    // t5
    ROTATE_LEFT_WORD(state[5], aux0, 8);
    ROTATE_LEFT_WORD(state[5], aux1, 16);
    XOR(aux0, aux1, t[5]);

    // t6
    ROTATE_LEFT_WORD(state[6], aux0, 8);
    XOR(state[6], aux0, t[6]);

    // t7
    ROTATE_LEFT_WORD(state[7], aux0, 8);
    ROTATE_LEFT_WORD(state[7], aux1, 16);
    XOR(aux0, aux1, t[7]);

    XOR(state[2], t[1], state[2]);
    XOR(t[1], t[0], t[1]);
    ROTATE_LEFT_WORD(t[1], t[1], 8);
    XOR(state[1], t[1], state[1]);
    ROTATE_LEFT_WORD(t[0], t[0], 16);
    XOR(state[0], t[0], state[0]);
    XOR(state[4], t[3], state[4]);
    XOR(t[3], t[2], t[3]);
    ROTATE_LEFT_WORD(t[3], t[3], 8);
    XOR(state[3], t[3], state[3]);
    ROTATE_LEFT_WORD(t[2], t[2], 16);
    XOR(state[2], t[2], state[2]);
    XOR(state[6], t[5], state[6]);
    XOR(t[5], t[4], t[5]);
    ROTATE_LEFT_WORD(t[5], t[5], 8);
    XOR(state[5], t[5], state[5]);
    ROTATE_LEFT_WORD(t[4], t[4], 16);
    XOR(state[4], t[4], state[4]);
    XOR(state[0], t[7], state[0]);
    XOR(state[1], t[7], state[1]);
    XOR(state[3], t[7], state[3]);
    XOR(state[4], t[7], state[4]);
    XOR(t[7], t[6], t[7]);
    ROTATE_LEFT_WORD(t[7], t[7], 8);
    XOR(state[7], t[7], state[7]);
    ROTATE_LEFT_WORD(t[6], t[6], 16);
    XOR(state[6], t[6], state[6]);
}
...
void encrypt(uint32_t state[][CHANNELS], uint32_t key[][CHANNELS]) {
    ...
    for (int i = 0; i < ROUNDS_NO - 1; i++) {
        ...
        mix_columns(state);
        add_round_key(state, &key[8*(i+1)]);
    }

    sub_bytes(state);
    shift_rows(state, true);
    add_round_key(state, &key[80]);
}
